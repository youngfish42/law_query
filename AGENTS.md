...
├── mcp_backfill_state.json           # MCP 覆盖账本（v2：月份×关键词 已确定覆盖的日区间）
...
- **类别归一化**：所有写入 CSV 的 `category` 必须经过 `normalize_category()` 处理；新增类别时同步更新 `CATEGORY_NAME_MAP`（`query.py`；`generate_rss.py` 直接使用 CSV 中的中文类别，无映射表）。
